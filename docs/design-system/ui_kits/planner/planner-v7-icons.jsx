/* Khonsera Planner v5 — icon set (Lucide subset) + time helpers.
 * Shared to window so planner-v5.jsx can consume. */
const P = {
  ...
  coach: ['<path d="M8 6v6"/>','<path d="M15 6v6"/>','<path d="M2 12h19.6"/>','<path d="M18 18h3s.5-1.7.8-2.8c.1-.4.2-.8.2-1.2 0-.4-.1-.8-.2-1.2l-1.4-5C20.6 6.8 20.3 6 19 6H4a2 2 0 0 0-2 2v10h3"/>','<circle cx="7" cy="18" r="2"/>','<path d="M9 18h5"/>','<circle cx="16" cy="18" r="2"/>'],
  eurostar: ['<path d="M2 12h20"/>','<path d="M4 9h12l4 3-4 3H4z"/>','<path d="M6 6v3"/>','<path d="M6 15v3"/>'],
  ferry: ['<path d="M2 21c.6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2 1.3 0 1.9.5 2.5 1 .6.5 1.2 1 2.5 1 2.5 0 2.5-2 5-2"/>','<path d="M4 18 2.3 9.6a1 1 0 0 1 .7-1.2L12 6l9 2.4a1 1 0 0 1 .7 1.2L20 18"/>','<path d="M12 10v4"/>','<path d="M12 2v4"/>'],
...
function Icon({ name, size = 16, sw = 1.75 }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
    strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round" style={{ flex: "none", display: "block" }}
    dangerouslySetInnerHTML={{ __html: (P[name] || []).join("") }} />;
}
function toMin(s) { const [h, m] = s.split(":").map(Number); return h * 60 + m; }
function fromMin(x) { x = ((x % 1440) + 1440) % 1440; return String(Math.floor(x / 60)).padStart(2, "0") + ":" + String(x % 60).padStart(2, "0"); }
function durLabel(mins) { const h = Math.floor(mins / 60), r = mins % 60; return h ? (r ? `${h}h ${r}m` : `${h}h`) : `${r} min`; }
function diff(a, b) { return durLabel(toMin(b) - toMin(a)); }

Object.assign(window, { P, Icon, toMin, fromMin, durLabel, diff });
