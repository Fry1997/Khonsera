...
Code session on** (per the Design↔Code Handoff Protocol). The effective source of truth is
`src/app/khonsera-instrument-tokens.css`, with responsive component composition in
`src/app/khonsera-instrument.css`. These import last so they supersede historical edition layers
without changing application contracts.

> **Instrument Edition v8 is live as of 2026-07-28.** The authority is Khonsera Design System 9:
> neutral stone ground, flat white instrument surfaces, signal-green interaction accent, rail-orange
> line identity, cool high-contrast ink and quiet single-direction elevation. Paper texture,
> letterpress, deboss and the pictorial moon mark are retired from app chrome.
...
> The instrument is **sans-only**: Satoshi carries display + UI + body and is self-hosted from
> `public/fonts`. Inter and the editorial serif are retired. The voice is operational —
> emphasis comes from weight, tracking and the mono technical layer, never a literary italic.

## Colour — Instrument light and Night Instrument
...
> **Effective light values:** ground `--paper` #f7f7f4; surface `--card` #ffffff; ink `--ink`
> #15171c; signal `--gold` #178a58; deep action block `--char` #0e2e27; route identity `--rail`
> #d95327. Compatibility names remain so existing components inherit the new direction. Night
> Instrument is `[data-theme=dark]`. Sahara is retired. The source file's duplicated appended light
> selector accidentally repeats night accent values; the imported token file removes that duplicate
> and retains these canonical light signal values.
...
`--radius-lg` 20 · `--radius-xl` 24 · `--radius-pill` 999. Cards use a consistent rounded-rectangle
language; pills stay fully round as deliberate compact controls.

## Motion

Durations `--dur-fast` 90ms · `--dur-base` 170ms · `--dur-slow` 240ms. Easings `--ease-standard`
`cubic-bezier(.32,.94,.36,1)` · `--ease-out` (entrance) · `--ease-in` (exit). Calm by default. Tailwind:
