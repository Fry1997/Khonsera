        ...
        {broadcasting ? (
          ...
          <div className="cc-share-live-form">
            ...
            <span className="cc-share-scope">
              {arriveIso ? <button type="button" data-active={scope === "arrive" ? "true" : "false"} onClick={() => setScope("arrive")}>Until I arrive</button> : null}
              <button type="button" data-active={scope === "today" ? "true" : "false"} onClick={() => setScope("today")}>For today</button>
              {multiDay && arriveIso ? <button type="button" data-active={scope === "trip" ? "true" : "false"} onClick={() => setScope("trip")}>For the trip</button> : null}
              <button type="button" data-active={scope === "custom" ? "true" : "false"} onClick={() => setScope("custom")}>Set hours</button>
            </span>
            {scope === "custom" ? <span className="cc-share-dur">{[1, 2, 4, 8].map((h) => <button key={h} type="button" data-active={customHours === h ? "true" : "false"} onClick={() => setCustomHours(h)}>{h}h</button>)}</span> : null}
        ...
        <ul className="cc-share-list">
          {active.map((s) => (
            <li key={s.id} className="cc-share-row">
              ...
              <span className="cc-share-row-meta">{liveToken === s.token ? "broadcasting" : s.lastAt ? `last sent ${new Date(s.lastAt).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}` : "ready"} · until {new Date(s.expiresAt).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</span>
